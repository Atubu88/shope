...
def upgrade() -> None:
    op.add_column("orders", sa.Column("name", sa.String(length=150), nullable=True))
    op.add_column("orders", sa.Column("email", sa.String(length=150), nullable=True))

    # Добавляем delivery_type с дефолтом, чтобы старые строки не ломались
    op.add_column(
        "orders",
        sa.Column(
            "delivery_type",
    ...
    # Обновляем старые NULL-значения перед тем как ставить NOT NULL
    ...
    op.execute("UPDATE orders SET payment_method = 'cash' WHERE payment_method IS NULL")
    op.alter_column(
        "orders", "payment_method", existing_type=sa.String(length=20), nullable=False
    )

    # Добавляем product_name в order_item
    op.add_column(
        "order_item",
        sa.Column("product_name", sa.String(length=150), nullable=False, server_default=""),
    )
    # убираем server_default после заполнения
