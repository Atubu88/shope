...
def get_currency_kb() -> InlineKeyboardMarkup:
    # Коды валют/флаги — статический текст (не переводим).
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="₽ RUB", callback_data="currency_RUB"),  # Россия
                InlineKeyboardButton(text="$ USD", callback_data="currency_USD"),  # доллары
                InlineKeyboardButton(text="€ EUR", callback_data="currency_EUR"),  # евро
            ],
            [
...
def get_tz_fixed_kb() -> InlineKeyboardMarkup:
    # Переводим подписи при построении, чтобы учитывалась текущая локаль.
    kb = [
        [InlineKeyboardButton(text=_(label), callback_data=f"tz_pick:{tz}")]
...
def contact_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text=_("📞 Отправить контакт"), request_contact=True)]],
...
async def start_via_invite(
    ...
    try:
        ...
        # Fallback for simple session stubs без execute()
...
async def invite_set_language_invalid(callback: types.CallbackQuery) -> None:
    await callback.answer(_("Выберите язык"), show_alert=True)
...
async def salon_name(message: types.Message, state: FSMContext) -> None:
    ...
    if len(name) < 2:
        await message.answer(_("Название слишком короткое, попробуйте ещё раз"))
        return
    await state.update_data(name=name)
    await message.answer(_("Введите slug или '-' для автоматического создания"))
...
async def salon_name_invalid(message: types.Message) -> None:
    await message.answer(_("Отправьте текстовое название или 'отмена'"))
...
async def salon_slug(message: types.Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await message.answer(_("Выберите валюту салона:"), reply_markup=get_currency_kb())
...
async def salon_slug_invalid(message: types.Message) -> None:
    await message.answer(_("Введите slug текстом или '-' для авто"))
...
async def salon_currency(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(_("Выберите ваш часовой пояс:"), reply_markup=get_tz_fixed_kb())
...
async def salon_currency_invalid(callback: types.CallbackQuery) -> None:
    await callback.answer(_("Используйте кнопки для выбора валюты"), show_alert=True)
...
async def tz_pick(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.delete()
    await callback.message.answer(
        _("Отправьте контакт владельца салона:"),
...
async def tz_invalid(callback: types.CallbackQuery) -> None:
    await callback.answer(_("Пожалуйста, выберите тайм-зону кнопками"), show_alert=True)
...
async def salon_phone(message: types.Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    try:
        salon = await SalonRepository(session).create_salon(
            name, slug, currency, timezone_name
        )
    except ValueError:
        await message.answer(
    ...
    await message.answer_photo(
        ...
        caption=_(
            ...
            "{link}"
        ).format(name=salon.name, timezone=salon.timezone, link=link),
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def salon_phone_invalid(message: types.Message) -> None:
    await message.answer(
        _("Пожалуйста, отправьте контакт кнопкой ниже"),
        reply_markup=contact_keyboard(),
    )
